      ...
      ,success: function(data){
    	  ...
    	  var data1 = new Array();
      	  for(var i=0;i<data.count;i++){
      		var clearScore = newData[i].clearScore;
      		var totalScore;
      		if(courseData.scoreType=="等级制"){
      			console.log(clearScore);
	      	    if(newData[i].clearScore>=60 && newData[i].clearScore<100) totalScore="D";
	      	    else if(newData[i].clearScore> 0  && newData[i].clearScore<60 ) totalScore="E";
	      	    else totalScore=clearScore;
	      	    console.log("+++++++")
	      	    console.log(totalScore);
      		}else{
      			if(clearScore>=60 && clearScore<100)
      				totalScore=60;
      			else
      	...
      	if(courseData.scoreType=="等级制"){
	    	...
	    	table1 = table.render({
	    	    ...
	    	    ,cols: [[ // 表头
	    	      ...
	    	      ,{field: 'clearScore', title: '补考成绩', width:120,align: 'center',style:'background-color: #eee;',
	    	    	  templet: function(d){
	    	    			  return d.totalScore;    // clearScore 与 totalScore 值相同
	    	    	  }
	    	    	}
  	    	...
  	    	table3 = table.render({
  	    	    ...
  	    	    ,cols: [[ // 表头
  	    	      ...
  	    	      ,{field: 'clearScore', title: '补考成绩',  align: 'center',
  	    	    	  templet: '#selectClearScore'
  	    	    	}
      	...
      	// 百分制
      	  ...
      	  table2=table.render({
      	    ...
      	    ,cols: [[ // 表头
      	      ...
      	      ,{field: 'sName', title: '学生姓名', width:120 , align: 'center'}
      	      ,{field: 'clearScore', title: '补考成绩', width: 120, align: 'center', edit:'text'}
      	      ,{field: 'totalScore', title: '总评成绩', width: 120, align: 'center'}
      	    ]]
      	    ,done:function(res,curr,count){
              // 关闭可编辑状态：
...
//	  
  ...
  table.on('edit(GradeInput)',function(obj){
	// 获取当前选中行的数据
  	...
  	var clearScore = parseInt(obj.data['clearScore']) ;
  	var totalScore1;
  	if(clearScore>=60)
  		totalScore1=60;
  	else
  		totalScore1=clearScore;
    ...
    if(courseData.entered==0){
    	...
    	if(courseData.scoreType=="等级制"){
        	for(var i=0;i<newData.length;i++){
        		  var tempScore;
        		  console.log("-----提交成绩-------")
        		  console.log(newData[i].clearScore);
        		  if(newData[i].clearScore>=60 && newData[i].clearScore<100 || newData[i].clearScore!='E')
        			  tempScore=60;
        		  else if(newData[i].clearScore<60 && newData[i].clearScore>=0 || newData[i].clearScore=='E')
        			  tempScore=50;
        		  else
        			  tempScore=-1;
	    		  newData[i].clearScore=tempScore;
	    		  console.log(newData[i].clearScore);
        	}
        }        
        $.ajax({
  ...
  form.on('select(_selectClearScore)', function(data){
	  // 更新缓存的值 
